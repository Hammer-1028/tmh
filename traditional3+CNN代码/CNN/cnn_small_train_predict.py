...
推荐先小样本测试：
python cnn_small_train_predict.py --data_root "D:/your_dataset" --max_samples 60 --train_samples 40 --epochs 30 --batch_size 2 --img_size 256
...
def build_pairs(data_root: Path, use_sets: List[str]) -> List[Tuple[Path, Path]]:
    """按文件 stem 匹配 Original 和 Label。"""
    pairs: List[Tuple[Path, Path]] = []
    for name in use_sets:
        img_dir = data_root / name / "Original"
        lab_dir = data_root / name / "Label"
        imgs = list_images(img_dir)
        labs = list_images(lab_dir)
        lab_map: Dict[str, Path] = {p.stem.lower(): p for p in labs}
...
def read_label_binary(path: Path, threshold: int = 20) -> np.ndarray:
    ...
    """
    lab = cv2.imread(str(path), cv2.IMREAD_COLOR)
    if lab is None:
...
def train(args):
    ...
    pairs = build_pairs(data_root, use_sets)
    if len(pairs) == 0:
        raise RuntimeError(
            "没有找到图像-标签配对。请检查目录是否为：data_root/Colour1/Original, data_root/Colour1/Label 等。"
        )
    ...
    # 小样本划分：默认前 train_samples 张训练，剩余验证；如果样本太少，则用训练集自己验证过拟合效果
    n = len(dataset)
...
def parse_args():
    ...
    parser.add_argument("--data_root", type=str, required=True,
                        help="数据根目录，例如 D:/TM_dataset，里面包含 Colour1/Original, Colour1/Label 等")
    parser.add_argument("--use_sets", type=str, default="Colour1,Colour2",
                        help="使用哪些数据集，默认 Colour1,Colour2")
