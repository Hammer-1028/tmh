...
def overlay_prediction(rgb: np.ndarray,
    ...
    cv2.circle(vis, (px, py), max(3, int(round(w / 160))), (255, 0, 0), -1)  # RGB 红点

    ys = np.where(pred_mask[:, px] > 0)[0]
    if len(ys) > 0:
        y1, y2 = int(ys.min()), int(ys.max())
        cv2.line(vis, (px, y1), (px, y2), (255, 255, 0), max(1, int(round(w / 300))))

    text1 = f"GT:{CLASS_NAMES[gt_class]}  Pred:{CLASS_NAMES[pred_class]}"
    text2 = f"TMH={pred_tmh_px:.1f}px"
    # OpenCV 默认不支持中文，这里只写简单英文/数字，避免乱码
    cv2.putText(vis, f"GT:{CLASS_SHORT[gt_class]} Pred:{CLASS_SHORT[pred_class]}", (8, 22),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    cv2.putText(vis, text2, (8, 45), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
...
def train_one_epoch(model: nn.Module,
    ...
    for batch in loader:
        ...
        point_heat = batch["point_heatmap"].to(device, non_blocking=True)
        meniscus_mask = batch["meniscus_mask"].to(device, non_blocking=True)
...
def evaluate_split(model: nn.Module,
    ...
    with torch.no_grad():
        for batch in loader:
            img_tensor = batch["image"].to(device, non_blocking=True)
